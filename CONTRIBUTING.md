...
1. New file under `src/sentinel/features/<block>.py`. Export a
   ...
   (`tech_*`, `reddit_*`, `twitter_*`). Prefixes are load-bearing, since
   the ablation harness partitions features by prefix.
...
- No vendored models, no committed datasets, no committed credentials.
  `.dockerignore` and `.gitignore` enforce this, please don't go around
  them.
