...
State lives on the named volume `sentinel_data` mounted at `/data`. It
survives deploys, and survives VM swaps. The DuckDB file, MLflow local
runs, and any CLI-generated reports all land there.

### Upgrading to Postgres on Fly

1. `fly pg create` for a standard Fly Postgres cluster.
